...
from ToolsActiveLearning import data_shape, data_vstack
...
def select_instance(X_training, X_pool, X_uncertainty: np.ndarray,
                    mask: np.ndarray,
...
def uncertainty_batch_sampling(classifier,
                               X,
                               X_labelled,
                               n_instances: int = 20,
    ...
    temp = SelectionFunction()
    uncertainty = temp.classifier_uncertainty(classifier=classifier, X=X, **uncertainty_measure_kwargs)
    return ranked_batch(classifier, unlabeled=X, X_training=X_labelled, uncertainty_scores=uncertainty,
